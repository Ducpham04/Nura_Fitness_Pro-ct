...
export const adminModules: AdminModuleConfig[] = [
  ...
  },
  {
    ...
    tableColumns: ['id', 'name', 'rewardType', 'creditValue', 'points', 'total', 'status', 'externalPartner'],
    fields: [
      { name: 'name',            label: 'Tên phần thưởng', required: true },
      { name: 'description',     label: 'Mô tả',           type: 'textarea' },
      { name: 'rewardType',      label: 'Loại phần thưởng', type: 'select', required: true,
        options: ['PHYSICAL', 'CREDIT'],
        hint: 'PHYSICAL = quà vật lý/voucher (admin giao tay). CREDIT = cộng credit AI ngay lập tức.' },
      { name: 'creditValue',     label: 'Số credit AI',    type: 'number',
        hint: 'Chỉ dùng khi Loại = CREDIT. Số credit AI được cộng vào tài khoản user khi đổi.' },
